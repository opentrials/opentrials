...
function searchPage(request, reply) {
  ...
  const advancedSearch = (Object.keys(filters).length > 0);

  fdaDocuments.search(queryStr, page, perPage, filters)
    .then((docs) => {
...
function getFilters(query) {
  ...
  if (query.name) {
    filters.name = escapeElasticSearch(query.name);
  }
...
module.exports = {
  ...
  validate: {
    query: {
      ...
      action_date_start: Joi.date().format('YYYY-MM-DD').empty('').raw(),
      action_date_end: Joi.date().format('YYYY-MM-DD').empty('').raw(),
