...
function searchPage(request, reply) {
  ...
  fdaDocuments.search(queryStr, textQuery, page, perPage, filters)
    .then((docs) => {
      ...
      const queryDescription = queryStr || 'FDA documents';
      const title = `Search results for ${queryDescription} in OpenTrialsFDA`;
      const description = `OpenTrialsFDA has ${docs.total_count} matches for
